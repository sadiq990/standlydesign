...
const DayFlow = ({ userData }) => {
    ...
    const schedule = React.useMemo(() => {
        if (!userData || !userData.wakeTime) {
            return [
                { id: 1, time: '07:30', label: 'Sabahın xeyir' },
                { id: 2, time: '09:00', label: 'Ofisə çat' },
                { id: 3, time: '12:00', label: 'Nahar fasiləsi' },
                { id: 4, time: '18:00', label: 'İş bitir' },
                { id: 5, time: '21:00', label: 'Evə qayıt' },
            ];
        }

        return [
            { time: userData.wakeTime, label: 'Səhər oyanışı' },
            { time: userData.workStart, label: 'İş moduna keçid' },
            { time: userData.lunchTime, label: 'Nahar fasiləsi' },
            { time: userData.workEnd, label: 'Günün sonu' },
            { time: userData.sleepTime, label: 'Yuxu vaxtı' },
        ].filter(e => e.time).sort((a, b) => timeToMins(a.time) - timeToMins(b.time)).map((e, idx) => ({ ...e, id: idx + 1 }));
    }, [userData]);

    const events = schedule.map(e => ({ ...e, mins: timeToMins(e.time) }));
